...
from tensorflow.keras.layers import (Input, Conv2D, BatchNormalization, Activation, MaxPooling2D,
                                     UpSampling2D, Multiply, Add, concatenate, Flatten, Dense, Reshape)
...
class QuantumLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        quantum_results.set_shape([None, n_qubits])  # shape!
...
def attention_unet_with_quantum_layer(IMG_CHANNELS, LearnRate):
    ...
    d1 = decoder_block(quantum_output, s4, 128)
    ...
    model_atten.compile(optimizer=Adam(learning_rate=LearnRate),
                        loss=bce_dice_loss,
...
IMG_CHANNELS = 3
LearnRate = 0.001
model = attention_unet_with_quantum_layer(IMG_CHANNELS, LearnRate)
